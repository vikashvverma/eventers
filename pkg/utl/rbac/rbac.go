...
func (s *Service) User(c echo.Context) *eventers.AuthUser {
	id := c.Get("id").(int)
	companyID := c.Get("company_id").(int)
	locationID := c.Get("location_id").(int)
	user := c.Get("username").(string)
	email := c.Get("email").(string)
	role := c.Get("role").(eventers.AccessRole)
	return &eventers.AuthUser{
		ID:         id,
		Username:   user,
		CompanyID:  companyID,
...
func (s *Service) EnforceRole(c echo.Context, r eventers.AccessRole) error {
	return checkBool(!(c.Get("role").(eventers.AccessRole) > r))
}
...
func (s *Service) EnforceCompany(c echo.Context, ID int) error {
	...
	if err := s.EnforceRole(c, eventers.CompanyAdminRole); err != nil {
		return err
	}
...
func (s *Service) EnforceLocation(c echo.Context, ID int) error {
	...
	if err := s.EnforceRole(c, eventers.LocationAdminRole); err != nil {
		return err
	}
...
func (s *Service) isAdmin(c echo.Context) bool {
	return !(c.Get("role").(eventers.AccessRole) > eventers.AdminRole)
}

func (s *Service) isCompanyAdmin(c echo.Context) bool {
	// Must query company ID in database for the given user
	return !(c.Get("role").(eventers.AccessRole) > eventers.CompanyAdminRole)
}
...
func (s *Service) AccountCreate(c echo.Context, roleID eventers.AccessRole, companyID, locationID int) error {
...
func (s *Service) IsLowerRole(c echo.Context, r eventers.AccessRole) error {
	return checkBool(c.Get("role").(eventers.AccessRole) < r)
}
