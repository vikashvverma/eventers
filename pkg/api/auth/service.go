...
type Service interface {
	Authenticate(echo.Context, string, string) (*eventers.AuthToken, error)
	Refresh(echo.Context, string) (*eventers.RefreshToken, error)
	Me(echo.Context) (*eventers.User, error)
}
...
type UserDB interface {
	...
	FindByUsername(orm.DB, string) (*eventers.User, error)
	FindByToken(orm.DB, string) (*eventers.User, error)
	Update(orm.DB, *eventers.User) error
}

// TokenGenerator represents token generator (jwt) interface
type TokenGenerator interface {
	GenerateToken(*eventers.User) (string, string, error)
}
