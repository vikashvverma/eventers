...
import (
	...
	"github.com/vikashvverma/eventers/pkg/api/auth/transport"
...
func TestLogin(t *testing.T) {
		...
		},
		{
			...
			udb: &mockdb.User{
				FindByUsernameFn: func(orm.DB, string) (*eventers.User, error) {
					return nil, eventers.ErrGeneric
				},
			},
		},
		{
			...
			udb: &mockdb.User{
				FindByUsernameFn: func(orm.DB, string) (*eventers.User, error) {
					return &eventers.User{
						Password: "hunter123",
						Active:   true,
					}, nil
				},
				UpdateFn: func(db orm.DB, u *eventers.User) error {
					return nil
				},
			},
			jwt: &mock.JWT{
			...
			wantResp: &eventers.AuthToken{Token: "jwttokenstring", Expires: mock.TestTime(2018).Format(time.RFC3339), RefreshToken: "refreshtoken"},
		},
	}
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantResp != nil {
				response := new(eventers.AuthToken)
...
func TestRefresh(t *testing.T) {
	cases := []struct {
		...
		req        string
		wantStatus int
		wantResp   *eventers.RefreshToken
		udb        *mockdb.User
		jwt        *mock.JWT
	}{
		{
		...
		},
		{
			...
			wantResp: &eventers.RefreshToken{Token: "jwttokenstring", Expires: mock.TestTime(2018).Format(time.RFC3339)},
		},
	}
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			defer res.Body.Close()
			if tt.wantResp != nil {
				response := new(eventers.RefreshToken)
				if err := json.NewDecoder(res.Body).Decode(response); err != nil {
					t.Fatal(err)
				}
				assert.Equal(t, tt.wantResp, response)
...
func TestMe(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			wantStatus: http.StatusOK,
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, i int) (*eventers.User, error) {
					return &eventers.User{
						Base: eventers.Base{
							ID: i,
						},
