...
func NewHTTP(svc event.Service, er *echo.Group) {
	...
	// swagger:operation GET /v1/events events listUsers
	// ---
	...
	// swagger:operation GET /v1/events/{id} events getUser
	// ---
...
type listResponse struct {
	Events []eventers.Event `json:"events"`
	Page   int              `json:"page"`
}

func (h *HTTP) list(c echo.Context) error {
	p := new(eventers.PaginationReq)
...
type updateReq struct {
	...
	Date     *mssql.DateTime1 `json:"date,omitempty" validate:"omitempty"`
	Location *string          `json:"mobile,omitempty"`
}
